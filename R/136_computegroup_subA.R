...
fnCornerpoints <- function(df, siiparams)
    {
      ...
      ## ri : radius inner
      ## ro : radius outer
      ## db : degrees beginning of section (compaswise: 0 degrees is north, 90 degrees is east)
      ## de : degrees end of section

      ## values outlinetype(assigned in function 'polygonpoints') are 1 through 4 and 11 and 13.
      ...
      ## types 11 and 13 are similar to 1 and 3, but indicate the outer radii of segments belonging to the same level.
            ...
            )                                   )
          {
            if (levelcounter == "1")
            ...
            else ## levelcounter != "1"
              {
                ...
                ## linestoprocess <- linestoprocess[!is.na(linestoprocess$level),]
                    ...
                    if (linestoprocess_ind[linecounter] == TRUE)
                      {
                        ...
                        if (processedlines == numberoflinesgroup)
                          { de <- degroup }# to avoid rounding errors
                        else
...
fnRotation <- function(df, siiparams)
    {
      ...
      ## end parameters
        ...
        if (is.null(rotationdescription))
          ...
          { rotationA <- 360-df$db[df$description == rotationdescription]
            print(paste0("for id = ",df$id[1]," the description dependent rotation is : ", rotationA, " degrees"))
          }
        if (is.null(rotationdegrees))
          {rotationB <- 0}
        else
             {rotationB <- rotationdegrees}
        df_out<-df
        df_out$db <- df_out$db + rotationA + rotationB
        df_out$de <- df_out$de + rotationA + rotationB
        ...
        for (i in 1:nrow(df))
          { line <- df[i,]
            if (squared == TRUE)
              {
            ## CALL ##
                points <- fnSquarepoints(ri=line$ri, ro=line$ro, db =line$db, de =line$de,
                                           indication11 = FALSE,
                                           indication13 = FALSE,
                                           plottype = line$plottype
                                           # ,stepsize    ## default value 0.5 will be used
                                        )
              }
            else
              {
            ## CALL ##
                points <- fnCirclepoints(ri=line$ri, ro=line$ro, db =line$db, de =line$de,
                                          indication11 = FALSE,
                                          indication13 = FALSE,
                                          plottype = line$plottype
                )
          ...
          df1$polyorder <- seq(from = counter_polyorder + 1, by=1, length.out = nrow(points))
