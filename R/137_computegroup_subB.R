...
fnCirclepoints <- function(ri, ro, db , de , plottype, indication11 , indication13 ,stepsize = 0.50)
    {
        if (plottype == "outer")
          { xpoint <- round(ri * cos(.5*pi - db /360 * 2 *pi),digits = 7)
            ...
            if (indication11 == FALSE)
              ...
              {outlinetype <- "11"}
            df <-data.frame (xpoint=xpoint, ypoint = ypoint, outlinetype= outlinetype)
            ...
            if (indication11 == TRUE)
              ...
              {outlinetype <- "1"}
            df2 <-data.frame (xpoint=xpoint, ypoint = ypoint, outlinetype= outlinetype)
            df <- rbind(df,df2)
          }
        else
      ...
      ## points along the outer edge
        ...
        outlinetype <- "2"
        tmp <- data.frame(xpoint=xpoint, ypoint = ypoint, outlinetype= outlinetype)
        df <- rbind (df,tmp)
...
fnSquarepoints <- function(ri, ro, db , de , plottype, indication11 , indication13 )
    {

      ## a "radial line"
        ## determination of octants (compas bearing 0-45: octant 0, 45-90: octant 2 etc until octant 7)
          octant_b = ((db+2*360) %/% 45) %% 8
          octant_e = ((de+2*360) %/% 45) %% 8
# print(paste0("db de" , db, ", ",de))
# print(paste0("octants" , octant_b, ", ", octant_e))
        ## (beginning xpoint is calculated as xbsign * ri + (1-abs(xbsign) * sqrt(2*ri^2) * sin/cosin function :
        ## analog for endpoint (xesign, )
        ##       xbsign is -1/0/1, xsignB = 0 or 1, negative values are introduced by sin/cosine function
...
# print(paste0(xsignA,xsignB,ysignA,ysignB))
        ...
        if (plottype == "outer")
          { xpoint <- fnsqXP(vertdistance=ri, degrees=db, xsign=xbsign, rounding=7)
            ypoint <- fnsqYP(vertdistance=ri, degrees=db, ysign=ybsign, rounding=7)
            if (indication11 == FALSE)
              {outlinetype <- "1"}
            else
              {outlinetype <- "11"}
            df <-data.frame (degrees = db, afstand = ri, xpoint=xpoint, ypoint = ypoint, outlinetype= outlinetype)
            xpoint <- fnsqXP(vertdistance=ro, degrees=db, xsign=xbsign, rounding=7)
            ypoint <- fnsqYP(vertdistance=ro, degrees=db, ysign=ybsign, rounding=7)
            if (indication11 == TRUE)
              {outlinetype <- "11"}
            else
              {outlinetype <- "1"}
            df2 <-data.frame (degrees = db, afstand = ro, xpoint=xpoint, ypoint = ypoint, outlinetype= outlinetype)
            df <- rbind(df,df2)
          }
        else
          { #no point, but an empty dataframe
            ...
            ypoint <- 1 # dummy
            outlinetype <- "dummy"
            df <-data.frame (degrees = 0, afstand = 0, xpoint=xpoint, ypoint = ypoint, outlinetype= outlinetype)
            df <- df[0]
          }
...
# print(degseq)
        for (degreesloop in degseq)
          { octant_loop = ((degreesloop+2*360) %/% 45) %% 8
            ...
            xpoint <- fnsqXP(vertdistance=ro, degrees=degreesloop, xsign=xloopsign, rounding=7)
            ypoint <- fnsqYP(vertdistance=ro, degrees=degreesloop, ysign=yloopsign, rounding=7)
            outlinetype <- "2"
            tmp <- data.frame(degrees = degreesloop, afstand = ro, xpoint=xpoint, ypoint = ypoint, outlinetype= outlinetype)
            df <- rbind (df,tmp)
          }

      ## radial line
        if (plottype == "outer")
          { xpoint <- fnsqXP(vertdistance=ro, degrees=de, xsign=xesign, rounding=7)
            ypoint <- fnsqYP(vertdistance=ro, degrees=de, ysign=yesign, rounding=7)
            if (indication13 == TRUE)
              {outlinetype <- "13"}
            else
              {outlinetype <- "3"}
            tmp <- data.frame(degrees = de, afstand = ro, xpoint=xpoint, ypoint = ypoint, outlinetype= outlinetype)
            df <- rbind (df,tmp)

            xpoint <- fnsqXP(vertdistance=ri, degrees=de, xsign=xesign, rounding=7)
            ypoint <- fnsqYP(vertdistance=ri, degrees=de, ysign=yesign, rounding=7)
            if (indication13 == TRUE)
              {outlinetype <- "13"}
            else
              {outlinetype <- "3"}
            tmp <- data.frame(degrees = de, afstand = ri, xpoint=xpoint, ypoint = ypoint, outlinetype= outlinetype)
            df <- rbind (df,tmp)
          }
        else
          {
      ...
      ## points along the inner edge
        if (plottype == "outer")
        {
          degseq <- fnsqDegseq(startdegrees= db,enddegrees= de,counterclockwise= TRUE)
# print("degseq 4")
# print(degseq)
            for (degreesloop in degseq)
              { octant_loop = ((degreesloop+2*360) %/% 45) %% 8
                ...
                xpoint <- fnsqXP(vertdistance=ri, degrees=degreesloop, xsign=xloopsign, rounding=7)
                ypoint <- fnsqYP(vertdistance=ri, degrees=degreesloop, ysign=yloopsign, rounding=7)
                outlinetype <- "4"
                tmp <- data.frame(degrees=degreesloop,afstand=ri, xpoint=xpoint, ypoint = ypoint, outlinetype= outlinetype)
                df <- rbind (df,tmp)
              }
          }
      ## RESULTS
        polygonpoints_result <- df
# print(polygonpoints_result)
        return(polygonpoints_result)
    }
...
fnsqXP <- function (vertdistance, degrees, xsign, rounding)
    {
        xpoint <- round( xsign * vertdistance + (1-abs(xsign)) * sqrt(2*vertdistance^2) * cos(.5*pi - degrees /360 * 2 *pi), digits = rounding)
      ##return results
        return(xpoint)
    }
...
fnsqDegseq <- function(startdegrees,enddegrees, counterclockwise = FALSE, reset= FALSE)
    {
        if (reset == TRUE)
        ...
        if (counterclockwise == FALSE)
          { if (lastmultiple < firstmultiple)
              ...
              {degseq <- c(startdegrees, seq(from = firstmultiple, to = lastmultiple, by = 45) , enddegrees)}
          }
        else
          { if (lastmultiple < firstmultiple)
              {degseq <- c(enddegrees, startdegrees) }
            else
              {degseq <- c(enddegrees, seq(from = lastmultiple, to = firstmultiple, by = -45) , startdegrees)}
          }
      ##return results
        return(degseq)
    }
