...
fnComputegroup <- function(data,scales,siiparams)
    {
      ...
      ## geomsiidatatopoints returns a list with $df (and possible other elements)
      ## return results
        return(d_out)
    }
...
geomsiidatatopoints <- function(df,siiparams)
    {
      ...
      ## CALL ##
        ...
        cornerpoints_intresult <- fnCornerpoints(df = df,siiparams = siiparams ) ## ri,ro,db,de are determined
      ## CALL ##
        rotation_intresult <- fnRotation(df=cornerpoints_intresult,siiparams=siiparams)
        if (squared == TRUE)
          {
          ## CALL ##
...
# z_step5_intresult <<- step5_intresult
      ...
      ## CALL ##
              ...
              # levelordescription outline1 outline2 outline3 outline4 outline11 outline 13
              # 1                   NA       TRUE     NA       ...
              #
              # into
            ...
            ## first: drawoutline is TRUE or FALSE where explicitly set to T or F for a certain description.
              outlinedf_trans_description <- dplyr::rename(outlinedf_trans,description=levelordescription)# , drawoutline_description = drawoutline)
              t1 <- merge(x = step5_result, y =outlinedf_trans_description, by =c("description","outlinetype"), all.x = TRUE)
              ...
              # for the undefined fraction a second step is to see if a T or F is set for a certain level
              outlinedf_trans_level <- dplyr::rename(outlinedf_trans,level=levelordescription )#,drawoutline_level = drawoutline)
              t1 <- merge(x = step5_undefined, y =outlinedf_trans_level, by =c("level","outlinetype"), all.x = TRUE)
        ...
        step5_result <- as.data.frame(step5_result[with(step5_result,order(id,-ordering_3,-group,polyorder)),])

      ## RESULTS
        solviipolygon <- list(df = step5_result)
            ##for debugging
            # solviipolygon <- list(df = step5_result, step4_lastintresult = step4_intresult , step5_lastintresult = step5_intresult )
...
transform_outlinedf <- function(df)
    {
        result <- tidyr::gather(data = df, key = outlinetype , value = drawoutline, -levelordescription)
