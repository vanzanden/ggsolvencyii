...
fn_structure_expansion <- function (params)
    {
        ...
        ##making additional lines in structure for ## which levels are children and so are possible prone to grouping? all levels other then 1 !
        groupinglines <- fn_determinelevels(s_out$level)
        groupinglines <- groupinglines[groupinglines != "1"]
        ## description of grouping lines is deducted from description of higher level combined with aggregatesuffix
        groupinglines <- as.data.frame(groupinglines)
        colnames(groupinglines) <- "level_grouping"
        ## next line adds a column description, filled with the name of the level above the current
        groupinglines <- merge(x = groupinglines, y=s_out, all.X = TRUE, by.x = "level_grouping", by.y = "childlevel")
        ## populating columns
        ### can next three lines be combined ?
        groupinglines <- dplyr::select(groupinglines, description, level =level_grouping )
        groupinglines$description <- paste0(groupinglines$description,aggregatesuffix)
        ...
        ## initial location of "o"-lines is just after last item of that level
        ...
        ## now to remove the lines where we know for sure no aggregation will take place, so it will not show up in legend.
...
# print(length(levelmax))
        ...
        { levelmaxdf <-data.frame(level = t1$level, levelmax = rep(levelmax,nrow(t1)))}
        else
        { levelmaxdf <- as.data.frame(levelmax) }
# print(paste0("levelmaxdf:"))
# print(levelmaxdf)
      ## if the levelmax of a level is smaller than the amount of lines of that level which have a childlevel associated this gives issues
      ## first idea was to raise the maxlevel but this might result in a complex issue if childless levellines have higher values.
      ## It is solved in the actual grouping by an advanced ordering (not level,-value but level,childlevel,-value)
      ## see flag_levelmaxissue
...
fn_determinelevels <- function (vector_in,  ind_value = FALSE,
    ...
    {   v_out <- c(vector_in)
        ...
        if (ind_value == TRUE)      { ind_d <- FALSE;  ind_o <- FALSE;   ind_onlyspecials = FALSE}
...
fn_structure_data_integration <- function (expandedstructure,
                                           data)
    {
        ...
        ##  description(chr), level(chr), childlevel(chr), ordering_1 (int), ind_d(lgl), ind_o(lgl), ordering_2(int)
        ## columns in data (as returned by geom_..-call)
        ## x, y, description, value, group, (comparewithid), (PANEL), (FILLCOLOR), (COLOR), ...
        data <- dplyr::mutate(data, group=id)

        ## basis merge, this leaves "o" lines out of the picture
        d_out <- merge(x=data, y = expandedstructure, all.x= TRUE, by = "description")
        d_names <- colnames(d_out)
        d_lines <- nrow(d_out)

        ## we have to add lines for possible "o"-lines.
        ## the levels for which an "o" possibility exists
        s_t1 <- fn_determinelevels(expandedstructure$level, ind_d = FALSE, ind_o = TRUE,ind_onlyspecials = TRUE)
        if (!is.null(s_t1))
        { s_t1 <- as.data.frame(s_t1)
          colnames(s_t1) <- "level"
          s_t1$leveltmp <- sub("o","",s_t1$level)
          ## first we take a copy of basismerge, and remove each line which has an id and level equal to the next line,
          ##              or for the level there will never be a need for an "o"-line
          m_t1 <- d_out
          ...
          while (m_counter >= 2 )
          {
 # print(paste0(m_t1$description[m_counter-1]," -  ", m_t1$description[m_counter]," -  ", m_t1$id[m_counter-1] ," -  ", m_t1$id[m_counter] ," -  ", m_t1$level[m_counter-1] ," -  ", m_t1$level[m_counter]))
          ...
          m_t1$childlevel       <- NA
          m_t1$description      <- expandedstructure$description[match(m_t1$level,expandedstructure$level)]
          m_t1$value            <- 0
          m_t1$ind_d            <- FALSE
          m_t1$ind_o            <- TRUE
          m_t1$ordering_1       <- expandedstructure$ordering_1[match(m_t1$level,expandedstructure$level)]
          m_t1$ordering_2       <- expandedstructure$ordering_2[match(m_t1$level,expandedstructure$level)]
          m_t1$levelmax         <- 1

          ## COLOR and FILLCOLOR: most likely it will be connected to description, although user could have coupled it with id
          ## there might be a need to copy the properties of the smallest item of the corresponding level
          d_t2 <- d_out
          ## find smallest value for each id/level combination
          d_t2 <- d_t2  %>%
            dplyr::group_by(group,level) %>%
            dplyr::summarise(minvalue=min(value))
          d_t2 <- d_t2[d_t2$level %in% fn_determinelevels(d_t2$level, ind_d = FALSE, ind_o = FALSE),]
          ...
          ## loop over group and level, for each combination determine of grouping is neccesary,
          ## if so. calculate grouped value and mark lines to be included in final data or not
          # d_out$ind_show <- TRUE
          # d_out$ind_show[d_out$ind_d == TRUE] <- FALSE
          # d_out$ind_show[d_out$ind_o == TRUE] <- FALSE
          d_out <- fn_add_ind_show(d_out)

          ## each corepart of the nested for loop is a selection of d_out, which will be adjusted and pasted in a new dataframe: d_out2
          d_out2 <- d_out[1,]
          d_out2 <- d_out2[0,]

          for (g_counter in unique(d_out$group))
            { g_lines <- d_out[d_out$group == g_counter,]
              for (l_counter in fn_determinelevels(g_lines$level,ind_d=FALSE,ind_o=FALSE))
                { gl_lines <- g_lines[g_lines$level == l_counter,]
                  ...
                  if (paste0(l_counter,"o") %in% g_lines$level == TRUE)
                    ...
                    ## geen grouping possible if no of lines without components = 0 or 1, this check should be replaced to fn_structure_expansion
                      if (count_gllines_nochild <= 1 & (count_gllines_withchild+count_gllines_nochild>max_gllines))
                        { print(paste0("for level ", l_counter, " for id=", g_counter, " no accumulation is possible: only one levelcomponent has no childlevels, adjust parameter(dataframe) levelmax"))
                        ...
                        { ## flag_levelmaxissue
                          if (max_gllines<count_gllines_withchild+1)
                            ...
                            max_gllines <- count_gllines_withchild +1 ## we know count_gllines_nochild is greater or equal to 2
                              print(paste0("for level ", l_counter, " for id=", g_counter, " levelmax is adjusted from ", max_old , " to ",max_gllines, ", due to the amount of childlevels" ))
                              rm(max_old)
                            }
                          if (count_gllines > max_gllines)
                            { gl_lines_nochild <- gl_lines_nochild[order(-gl_lines_nochild$value ),]
                              ...
                              d_out2 <- rbind(d_out2,gl_lines_withchild,gl_lines_nochild_keep,gl_lines_nochild_tosum,o_line)
                            }
                          else
                            { d_out2 <- rbind(d_out2,gl_lines)}
                            }
        ...
        else ## er zijn in het geheel geen "o" rijen (dus is.null(s_t1) == TRUE)
        {
          d_out <- fn_add_ind_show(d_out)
          d_out2 <- d_out
