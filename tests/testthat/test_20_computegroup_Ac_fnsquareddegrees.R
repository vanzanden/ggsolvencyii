...
#    testparams$structure <- sii_z_ex1_structure
    ...
    testresultA <- ggsolvencyii:::fn_squareddegrees(circledegrees =  intresultD$db)  ## just one column, with degrees
    max(testresultA)
    min(testresultA)


 test_that("fn_squareddegrees" ,{
 expect_equal( max(testresultA) < 360, TRUE)
 expect_equal( min(testresultA) > -360, TRUE)
 })
