...
testparams$structuredf <- sii_structure_sf16_nld # sii_z_ex_structure
testparams$levelmax <- 99 # sii_levelmax_995 # sii_z_ex_levelmax
testparams$aggregatesuffix <- "_other"
testparams$plotdetails <- sii_plotdetails_sf16 # sii_z_ex4_plotdetails
...
x = tijd,
y = ratio,
                                                    id = id, value = waarde, description = description,
...
lwd = 0.5,
                          ...
                          ggplot2::theme_bw() +
                          ggplot2::scale_fill_manual(name = "Componenten", values = ggsolvencyii::sii_x_fillcolors_sf16_nld
                          ) +
                          ggplot2::scale_color_manual(name = "Componenten", values = ggsolvencyii::sii_x_edgecolors_sf16_nld
                          ) +
                          ggsolvencyii::geom_sii_riskoutline(data = testdata,
                                                            mapping = ggplot2::aes(
x = tijd,
y = ratio,
                                                            id = id, value = waarde, description = description, comparewithid = vergelijkmet),
color = "red",
lwd = 0.7,
alpha = 0.99,
...
x = tijd,
y = ratio,
                                                              id = id,comparewithid = vergelijkmet),
